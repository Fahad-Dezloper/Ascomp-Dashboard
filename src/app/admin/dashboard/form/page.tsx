...
const getInitialFieldConfigs = (): FieldConfig[] => {
  return [
    ...
    },
    {
      ...
      type: "text",
      required: true,
      section: "Projector Information",
    },
    {
      key: "projectorSerialNumber",
      label: "Serial Number",
      type: "text",
      required: true,
      section: "Projector Information",
    ...
    {
      key: "projectorPlacementEnvironment",
      label: "Projector Placement & Environment",
      type: "textarea",
      section: "Mechanical",
    },
    {
      ...
      type: "select",
      section: "Software & Screen Information",
    },
    {
      key: "screenHeight",
    ...
    {
      key: "throwDistance",
      label: "Throw Distance (m)",
      type: "number",
      section: "Software & Screen Information",
    },
    {
      key: "lampMakeModel",
      label: "Lamp Make & Model",
      type: "select",
      section: "Lamp Information",
    },
    {
    ...
    },
    {
      ...
      type: "select",
      section: "Content Player & AC Status",
    },
    {
      key: "acStatus",
      label: "AC Status",
    ...
    },
    {
      ...
      type: "select",
      section: "Content Player & AC Status",
      options: ["Removed", "Not removed – Good fL", "Not removed – De-bonded"],
    },
    {
      key: "focusBoresight",
      label: "Focus/Boresight OK",
      type: "select",
      section: "Image Evaluation",
      options: ["OK", "YES"],
    },
    {
      key: "integratorPosition",
      label: "Integrator Position OK",
      type: "select",
      section: "Image Evaluation",
      options: ["OK", "YES"],
    },
    {
      key: "spotsOnScreen",
      label: "Spots on Screen OK",
      type: "select",
      section: "Image Evaluation",
      options: ["OK", "YES"],
    },
    {
      key: "screenCroppingOk",
      label: "Screen Cropping OK",
      type: "select",
      section: "Image Evaluation",
      options: ["OK", "YES"],
    },
    {
      key: "convergenceOk",
      label: "Convergence OK",
      type: "select",
      section: "Image Evaluation",
      options: ["OK", "YES"],
    },
    {
      key: "channelsCheckedOk",
      label: "Channels Checked OK",
      type: "select",
      section: "Image Evaluation",
      options: ["OK", "YES"],
    },
    ...
    {
      key: "lightEngineSerialNumber",
      label: "Light Engine Serial Number",
      type: "text",
      section: "Remarks",
    },
  ];
...
export default function FormBuilderPage() {
  ...
  const [newLampModelValue, setNewLampModelValue] = useState<
    Record<string, string>
  >({});
  const [newProjectorModel, setNewProjectorModel] = useState("");

  const [selectedProjectorIndex, setSelectedProjectorIndex] = useState<
    number | null
  >(null);

  const [selectedPartsProjector, setSelectedPartsProjector] = useState<
    string | null
  >(null);
    ...
    const loadDataFiles = async () => {
      ...
      try {
        const results = await Promise.all([
          ...
          fetch("/api/admin/data-files/projector?t=" + Date.now(), {
            credentials: "include",
            cache: "no-store",
          }),
        ]);
        ...
        if (contentRes.ok) {
          ...
          console.error(
            "Failed to load content players:",
            contentRes.status,
            errorText,
          );
        }
        ...
        if (projectorRes.ok) {
          ...
          console.error(
            "Failed to load projector parts:",
            projectorRes.status,
            errorText,
          );
        }
  ...
  useEffect(() => {
    if (lampModelsData.length > 0) {
      if (selectedProjectorIndex === null) {
        ...
        selectedProjectorIndex >= lampModelsData.length
      ) {
        setSelectedProjectorIndex(0);
      }
    } else {
  ...
  const addOption = (fieldKey: string) => {
    ...
    updateField(fieldKey, {
      options: [
        ...(fieldConfigs.find((f) => f.key === fieldKey)?.options || []),
        option,
      ],
    });
  ...
  const removeOption = (fieldKey: string, optionIndex: number) => {
    ...
    updateField(fieldKey, {
      options: newOptions,
      subOptions:
        Object.keys(currentSubOptions).length > 0
          ? currentSubOptions
          : undefined,
      subOptionsInput:
        Object.keys(currentSubOptionsInput).length > 0
  ...
  const saveDataFile = async (
    fileType: "content-player" | "lamp-models" | "software" | "projector",
    values: any,
    silent = false,
  ) => {
    try {
      ...
      if (res.ok) {
        ...
        if (!silent) {
          toast.success(
            `Saved ${result.saved} ${fileType.replace("-", " ")} items`,
          );
        }
        if (fileType === "content-player")
          setContentPlayers(values as string[]);
        if (fileType === "lamp-models")
          setLampModelsData(
            values as Array<{ projector_model: string; Models: string[] }>,
          );
        if (fileType === "software") setSoftwareVersions(values as string[]);
        if (fileType === "projector")
          setProjectorPartsData(values as ProjectorPart[]);
      } else {
        const errorText = await res.text();
        ...
        toast.error(
          `Failed to save ${fileType}: ${errorData.error || "Unknown error"}`,
        );
      }
    } catch (error) {
      console.error(`Failed to save ${fileType}:`, error);
      toast.error(
        `Failed to save ${fileType}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };

  const addDataValue = (
    fileType: "content-player" | "lamp-models" | "software",
  ) => {
  ...
  const addProjectorPart = () => {
    if (
      ...
      !newPartValue.part_number ||
      !newPartValue.description
    )
      return;
  ...
  const removeProjectorPart = (partToRemove: ProjectorPart) => {
    const updated = projectorPartsData.filter(
      (p) =>
        !(
  ...
  const updateProjectorPart = (
    originalPart: ProjectorPart,
    field: keyof ProjectorPart,
    value: string,
  ) => {
  ...
  const removeProjectorModelFromParts = (model: string) => {
    if (!confirm(`Are you sure you want to delete all parts for ${model}?`))
      return;
    const updated = projectorPartsData.filter(
      (p) => p.projector_model !== model,
    );
    saveDataFile("projector", updated);
  ...
  const addLampModel = (projectorModelIndex: number) => {
    ...
    if (
      ...
      Array.isArray(updated[projectorModelIndex].Models)
    ) {
      updated[projectorModelIndex] = {
        ...updated[projectorModelIndex],
  ...
  const removeLampModel = (projectorModelIndex: number, modelIndex: number) => {
    ...
    if (
      ...
      updated[projectorModelIndex] = {
        ...updated[projectorModelIndex],
        Models: updated[projectorModelIndex].Models.filter(
          (_, i) => i !== modelIndex,
        ),
      };
      saveDataFile("lamp-models", updated);
  ...
  const updateLampModel = (
    projectorModelIndex: number,
    modelIndex: number,
    newValue: string,
  ) => {
    const updated = [...lampModelsData];
    if (
      ...
      updated[projectorModelIndex] = {
        ...updated[projectorModelIndex],
        Models: updated[projectorModelIndex].Models.map((model, i) =>
          i === modelIndex ? newValue : model,
        ),
      };
  ...
  const addProjectorModel = () => {
    ...
    const updated = [
      ...lampModelsData,
      { projector_model: projectorModel, Models: [] },
    ];
    saveDataFile("lamp-models", updated);
  ...
  const sectionDataFileMap: Record<
    string,
    "content-player" | "lamp-models" | "software" | "projector" | null
  > = {
  ...
  const selectTriggerClass =
    "h-7 text-xs font-medium rounded-md border-border/70 bg-muted/30 hover:bg-muted/50";
  const selectContentClass = "rounded-lg border-border/80 shadow-lg py-1";
  const inputCompact = "px-2 py-0.5";

  const handleOptionRename = (
    fieldKey: string,
    oldOption: string,
    newOption: string,
  ) => {
        ...
        <div className="flex items-center justify-between mb-5 pb-3 border-b border-border/60">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Customize form fields and dropdown options
            </p>
          </div>
          <Button
            onClick={saveConfig}
            size="sm"
            className="h-8 px-3 text-xs rounded-lg bg-primary text-primary-foreground hover:bg-primary/90"
          >
        ...
        <div className="space-y-3">
          ...
          {FORM_SECTIONS.map((section) => {
            ...
            return (
              <div
                key={section}
                className="border  border-border/60 bg-card shadow-sm rounded-lg overflow-hidden"
              >
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {fields.map((field) => (
                      <FormFieldConfigCard
                        ...
                        onToggleSubExpand={(option) =>
                          toggleSubOptionExpand(field.key, option)
                        }
                        onOptionRename={(oldOpt, newOpt) =>
                          handleOptionRename(field.key, oldOpt, newOpt)
                        }
                        newOptionValue={newOption[field.key] || ""}
                        onNewOptionChange={(v) =>
                          setNewOption((prev) => ({ ...prev, [field.key]: v }))
                        }
                        onAddOption={() => addOption(field.key)}
                        onRemoveOption={(idx) => removeOption(field.key, idx)}
                        getNewSubOptionValue={(k) => newSubOption[k] || ""}
                        onNewSubOptionChange={(k, v) =>
                          setNewSubOption((prev) => ({ ...prev, [k]: v }))
                        }
                        onAddSubOption={(opt) => addSubOption(field.key, opt)}
                        onRemoveSubOption={(opt, subIdx) =>
                          removeSubOption(field.key, opt, subIdx)
                        }
                      />
                    ))}
                  </div>
                    ...
                    <div className="mt-3 pt-3 border-t border-dashed border-border/50">
                      ...
                      <div className="flex flex-wrap gap-1.5 mb-2">
                        {softwareVersions.map((version, idx) => (
                          <div
                            key={idx}
                            className="flex items-center gap-1 bg-muted/60 px-2 py-1 rounded text-xs group"
                          >
                            <span>{version}</span>
                            <button
                              type="button"
                        ...
                        {contentPlayers.map((player, idx) => (
                          ...
                          >
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                removeDataValue("content-player", idx)
                              }
                              className="text-muted-foreground hover:text-destructive opacity-0 group-hover:opacity-100 transition-opacity"
                            >
                              <Trash2 className="h-2.5 w-2.5" />
                  ...
                  {associatedDataFile === "lamp-models" && (
                    <div className="mt-3 pt-3 border-t border-dashed border-border/50">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-[10px] text-muted-foreground">
                          {lampModelsData.length} projectors
                        </span>
                      </div>
                      <div className="flex gap-2 items-end">
                        <div className="flex-1 max-w-[200px]">
                          <Select
                            value={
                              selectedProjectorIndex !== null
                                ? String(selectedProjectorIndex)
                                : ""
                            }
                            onValueChange={(value) =>
                              setSelectedProjectorIndex(
                        ...
                        <div className="flex gap-1.5">
                          <Input
                            ...
                            onChange={(e) =>
                              setNewProjectorModel(e.target.value)
                            }
                          ...
                          <div className="mt-2 p-2 bg-muted/20 rounded-lg">
                            <div className="flex flex-wrap gap-1 mb-1.5">
                              {lampModelsData[
                                ...
                                <div
                                  key={modelIdx}
                                  className="flex items-center gap-1 bg-background/80 px-2 py-0.5 rounded text-xs group border border-border/50"
                                >
                                  <span>{model}</span>
                                  <button
                                    type="button"
                            ...
                            <div className="flex gap-1.5">
                              <Input
                                value={
                                  newLampModelValue[selectedProjectorIndex] ||
                                  ""
                                }
                                onChange={(e) =>
                                  setNewLampModelValue((prev) => ({
                                    ...prev,
                                    [selectedProjectorIndex]: e.target.value,
                                  }))
                                }
          ...
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-[10px] text-muted-foreground mt-0.5">
                Software, Content Players & Lamp Models are edited inline above.
                Parts below.
              </p>
            </div>
            <Button
              ...
              onClick={() => {
                ...
                const loadDataFiles = async () => {
                  try {
                    const results = await Promise.all([
                      fetch(
                        "/api/admin/data-files/content-player?t=" + Date.now(),
                        { credentials: "include", cache: "no-store" },
                      ),
                      fetch(
                        "/api/admin/data-files/lamp-models?t=" + Date.now(),
                        { credentials: "include", cache: "no-store" },
                      ),
                      fetch("/api/admin/data-files/software?t=" + Date.now(), {
                        credentials: "include",
                    ...
                    const [contentRes, lampRes, softwareRes, projectorRes] =
                      results;
            ...
            >
              <RefreshCw
                className={`h-3.5 w-3.5 mr-1.5 ${loadingDataFiles ? "animate-spin" : ""}`}
              />
              Refresh
            </Button>
          </div>
                  ...
                  {contentPlayers.map((value, idx) => (
                    <div key={idx} className="flex items-center gap-1.5">
                      <Input
                        value={value}
                        onChange={(e) =>
                          updateDataValue("content-player", idx, e.target.value)
                        }
                      ...
                      <Button
                        ...
                        className="h-6 w-6 p-0 rounded text-muted-foreground hover:text-destructive"
                      >
                        <Trash2 className="h-2.5 w-2.5" />
                      </Button>
                    </div>
                      ...
                      lampModelsData[selectedProjectorIndex] && (
                        <div className="border border-border/50 rounded-lg p-3 space-y-2">
                          ...
                          <div className="space-y-1.5">
                            {lampModelsData[selectedProjectorIndex].Models
                              ?.length ? (
                              <div className="space-y-1.5 max-h-48 overflow-y-auto">
                                {lampModelsData[
                                  selectedProjectorIndex
                  ...
                  {softwareVersions.map((value, idx) => (
                    <div key={idx} className="flex items-center gap-1.5">
                      <Input
                        value={value}
                        onChange={(e) =>
                          updateDataValue("software", idx, e.target.value)
                        }
                  ...
                  <>
                    <div className="flex items-center gap-2">
                      <Select
                        ...
                        onValueChange={(value) =>
                          setSelectedPartsProjector(value)
                        }
                      >
                        <SelectTrigger className={selectTriggerClass}>
                          <SelectValue placeholder="Projector..." />
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          removeProjectorModelFromParts(selectedPartsProjector!)
                        }
                        className="h-7 text-xs text-muted-foreground hover:text-destructive"
                        disabled={!selectedPartsProjector}
                      >
                      ...
                      <div className="border border-border/50 rounded-lg p-3 space-y-2">
                        ...
                        <div className="space-y-1.5 max-h-56 overflow-y-auto">
                          {projectorPartsData
                            .filter(
                              (p) =>
                                p.projector_model === selectedPartsProjector,
                            )
                            .map((part, pIdx) => (
                              <div
                                key={pIdx}
                              ...
                              >
                                <Input
                                  ...
                                  className={`col-span-2 text-xs border-border/60 rounded ${inputCompact}`}
                                ...
                                <Button
                                  ...
                                  className="col-span-1 h-6 w-6 p-0 rounded text-muted-foreground hover:text-destructive"
                                >
                                  <Trash2 className="h-2.5 w-2.5" />
                                </Button>
                              </div>
                        ...
                        <div className="grid grid-cols-5 gap-1.5 pt-2 border-t border-border/40">
                          ...
                          <Button
                            ...
                            className="col-span-1 h-6 w-6 p-0 rounded"
                            disabled={
                              !newPartValue.description ||
                              !newPartValue.part_number
                            }
                    ...
                    <div className="flex items-center gap-1.5 pt-2 border-t border-border/40">
                      <Input
                        ...
                        onChange={(e) =>
                          setNewPartsProjectorModel(e.target.value)
                        }
                        placeholder="New projector model"
                        className={`text-xs border-border/60 flex-1 rounded-md ${inputCompact}`}
                        onKeyDown={(e) => {
